...
const TrendingItem = ({ activeItem, item }) => {
  ...
  return (
    <Animatable.View className="mr-5" animation={activeItem == item.$id ? zoomIn : zoomOut}>
      {
        play ? (
          <Video source={{ uri: item.video }} className="w-52 h-72 rounded-[35px] mt-3 bg-white/10" resizeMode={ResizeMode.CONTAIN} useNativeControls shouldPlay onPlaybackStatusUpdate={(status)=> {
